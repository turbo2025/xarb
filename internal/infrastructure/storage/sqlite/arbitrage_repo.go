...
// SaveFuturesPrice 保存期货价格
func (ar *ArbitrageRepo) SaveFuturesPrice(ctx context.Context, price *model.FuturesPrice) error {
	_, err := ar.db.ExecContext(ctx, `
...
func (ar *ArbitrageRepo) GetLatestPrice(ctx context.Context, exchange, symbol string) (*model.FuturesPrice, error) {
	row := ar.db.QueryRowContext(ctx, `
