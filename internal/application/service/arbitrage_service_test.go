...
type MockArbitrageRepo struct {
	...
	prices   map[string]*model.FuturesPrice
}
...
func NewMockArbitrageRepo() *MockArbitrageRepo {
	return &MockArbitrageRepo{
		...
		prices:   make(map[string]*model.FuturesPrice),
	}
}
...
func (m *MockArbitrageRepo) SaveFuturesPrice(ctx context.Context, price *model.FuturesPrice) error {
	m.prices[price.Exchange+"_"+price.Symbol] = price
	return nil
}

func (m *MockArbitrageRepo) GetLatestPrice(ctx context.Context, exchange, symbol string) (*model.FuturesPrice, error) {
...
func TestSpreadArbitrage(t *testing.T) {
	...
	binancePrice := &model.FuturesPrice{
		Exchange:  "binance",
		Symbol:    "BTCUSDT",
		Price:     43000,
		Funding:   0.0001,
